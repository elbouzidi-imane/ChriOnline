...
public class OrderHandler {
    ...
    private Message handlePlaceOrder(Message req) {
        try {
            ...
            if (parts.length < 4) {
                return Message.error("Format invalide. Attendu : userId|adresse|modePaiement|modeLivraison");
            }
            ...
            Order order = orderService.validerCommande(userId, adresse, modePaiement, modeLivraison);
            if (order == null) {
                return Message.error("Impossible de valider la commande (panier vide ou stock insuffisant)");
            }
